// Public signal vector (LOCKED): [ note, nf_order, pair_id, batch_id, root, expiry, maq, tier ]
pragma circom 2.0.0;

// Crossed dark pool — ORDER circuit (Phase 2).
...
//
// v2 scopes notes to expiry, minimum acceptable quantity, and counterparty tier.
...
template Order(depth) {
    ...
    signal input expiry;        // u64 order expiry
    signal input maq;           // u64 minimum acceptable quantity
    signal input tier;          // u32 counterparty class
    ...
    // batch_id, expiry, maq, and tier are bound IN so a note is cryptographically scoped to them.
