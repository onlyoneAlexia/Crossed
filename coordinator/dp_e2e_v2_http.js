// Live two-party v2 dark-pool e2e over the coordinator HTTP API.
// Mirrors the FE flow: fund -> trustlines -> mint -> register(owner=trader) -> deposit
// -> client proves order_v2 -> POST /dp/order -> POST /dp/close -> balance assertions.
// Run with the coordinator up and DP_ORDER_V2 enabled.
import * as StellarSdk from "@stellar/stellar-sdk";

import { DEFAULT_DP_CONTRACT_ID } from "./chain.js";
import { buildTreeFromLeaves, identityFromSk, be32, proveOrderV2 } from "./darkpool.js";
...
const DP = process.env.DP_CONTRACT_ID ?? DEFAULT_DP_CONTRACT_ID;
const TOKENS = {
...
const TOKEN_A = TOKENS[BASE_SYM];
const TOKEN_B = TOKENS[QUOTE_SYM];
const ISSUER = process.env.TOKEN_ISSUER ?? "GDQPLQXZJWFGSVWM4JYCBXFOEAATO5TNH2MR674MABIBI5WU3LWTLOUK";
const MAQ = process.env.DP_V2_MAQ ?? "1";
const SELL_TIER = Number(process.env.DP_V2_SELL_TIER ?? 1);
const BUY_TIER = Number(process.env.DP_V2_BUY_TIER ?? 1);
...
async function buildRegisterAuth(kp, id) {
  ...
  let nonce = 0n;
  for (const x of b) nonce = (nonce << 8n) | BigInt(x);
  nonce &= (1n << 62n) - 1n;
...
async function sacBalance(token, account) {
  let src;
  try { src = await srv.getAccount(account); } catch { return 0n; }
...
async function escrow(token, owner) {
  let src;
  try { src = await srv.getAccount(owner); } catch { return 0n; }
  const tx = new TransactionBuilder(src, { fee: BASE_FEE, networkPassphrase: PASS })
    .addOperation(Operation.invokeContractFunction({
      contract: DP,
      function: "escrow_balance",
      args: [Address.fromString(owner).toScVal(), Address.fromString(token).toScVal()],
    }))
    .setTimeout(30).build();
  const sim = await srv.simulateTransaction(tx);
  if (!rpc.Api.isSimulationSuccess(sim) || !sim.result) return 0n;
...
async function buildOrderBody({ kp, id, sk, side, size, limit_price, salt, tier, pair_id = PAIR_ID, expiry }) {
  ...
  const order = await proveOrderV2({
    sk: sk.toString(),
    side,
    size,
    limit_price,
...
async function main() {
  if (!TOKEN_A || !TOKEN_B) throw new Error(`unknown token symbols: ${BASE_SYM}/${QUOTE_SYM}`);
  const health = await get("/health");
  if (!health.dp_order_v2) throw new Error("coordinator is not running with DP_ORDER_V2 enabled");

  const SIZE = "100000000";
  const LIMIT_SELL = "24000000";
  const LIMIT_BUY = "26000000";
  const QUOTE = "250000000";
  ...
  log("== SELLER setup ==");
  await setup(kpSell, idSell, TOKEN_A, "200000000");
  log("== BUYER setup ==");
  await setup(kpBuy, idBuy, TOKEN_B, "500000000");

  log("== deposits ==");
  log(`  seller deposits ${SIZE} ${BASE_SYM}`);
  await deposit(kpSell, TOKEN_A, SIZE);
  log(`  buyer deposits ${QUOTE} ${QUOTE_SYM}`);
  await deposit(kpBuy, TOKEN_B, QUOTE);
  ...
  log(`  escrow: seller ${BASE_SYM}=${escSellBaseBefore} buyer ${QUOTE_SYM}=${escBuyQuoteBefore}`);

  log("== submit v2 orders ==");
  const sellBody = await buildOrderBody({
    kp: kpSell, id: idSell, sk: skSell, side: 0, size: SIZE,
    limit_price: LIMIT_SELL, salt: salt(111), expiry, tier: SELL_TIER,
  });
  const oSell = await post("/dp/order", sellBody);
  log("  sell placed:", oSell.note?.slice(0, 12), "tx", oSell.tx?.slice(0, 10));
  const buyBody = await buildOrderBody({
    kp: kpBuy, id: idBuy, sk: skBuy, side: 1, size: SIZE,
    limit_price: LIMIT_BUY, salt: salt(222), expiry, tier: BUY_TIER,
  });
  const oBuy = await post("/dp/order", buyBody);
  log("  buy placed: ", oBuy.note?.slice(0, 12), "tx", oBuy.tx?.slice(0, 10));

  log("== close batch (dpmatch_v2 + settle_dp_match_v2) ==");
  ...
  const fill = closed.fills?.[0];
  if (!fill || fill.base_amount !== SIZE || fill.quote_amount !== QUOTE) {
    throw new Error(`unexpected v2 fill: ${JSON.stringify(fill)}`);
  }
  ...
  log(`  seller received quote: ${sellBBefore} -> ${sellBAfter}  (expect +${QUOTE})`);
  log(`  buyer received base:  ${buyABefore} -> ${buyAAfter}  (expect +${SIZE})`);
  ...
  if (!ok) throw new Error("v2 atomic swap balance assertions failed");
  log("\nLIVE DARK-POOL V2 HTTP E2E PASSED");
}

main().catch((e) => {
  console.error("V2 HTTP E2E ERROR:", e.message);
  process.exitCode = 1;
});
